...
//         , buffer_{}
      ...
      IPAddress ServerDiscovery::search_server()
      {
          if (socket.open()) {
              ...
              DynamicArray<network::IPAddress> found_addresses;


              while (/*!found && */found_addresses.size() < 2)
              {
                  stream.reset();
                  if (socket.receive(address, stream.buffer_, stream.length_))
                  {
                      NetworkStreamReader reader(stream);
                      if (reader.peek() == PROTOCOL_PACKET_CHALLENGE)
                      {
                          // Store address in a DynamicArray
